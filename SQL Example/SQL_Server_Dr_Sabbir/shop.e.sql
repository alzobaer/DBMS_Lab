...
insert customers values("ca0001","Ashraful","Haque","356 Suharawardi","017-24666730","jassor",0,0)
insert customers values("ca0002","Rafiqul","Islam","378 Suharawardi","017-38593820","jassor",0,0)
insert customers values("ca0003","Sifur","Rahman","322 Suharawardi","017-23536730","chuadanga",0,0)
insert customers values("ca0004","Nayan","Hasan","447 Mother box","017-54656853","chuadanga",0,0)
insert customers values("ca0005","Abul","Hasan","Amir Ali","017-34545320","Rajshahi",0,0)
insert customers values("ca0006","Selim","Reza","S.M","017-26766730","Chapi",0,0)
insert customers values("ca0007","Selina","Akter","Munnujan","017-16172325","Chuadanga",0,0)
insert customers values("ca0008","Sakir","Hosan","Lathif","017-45436730","Rajshahi",0,0)
insert customers values("ca0009","Fuad","Khan","Possim Para","017-23266730","Rajshahi",0,0)
insert customers values("ca0010","Julfiker","Ali","NASA","987-56326730","Rajshahi",0,0)
...
drop table items
insert items values("a0001","computer",3000,100,19-04-2008)
insert items values("a0002","Monitor",5000,100,19-04-2008)
insert items values("a0003","Sperer",2000,100,19-04-2008)
insert items values("a0004","Harddisk",3000,100,19-04-2008)
insert items values("a0005","RAM",1000,100,19-04-2008)
insert items values("a0006","Keybord",4000,100,19-04-2008)
insert items values("a0007","Mause",1000,100,19-04-2008)
insert items values("a0008","pendrive",5000,100,19-04-2008)
insert items values("a0009","Motherbord",6000,100,19-04-2008)
insert items values("a0010","CPU",4000,100,19-04-2008)
select *from items
delete from items
...
create trigger trg_transactions on transactions for insert
 as
   begin
     declare @tr_type char(1),@it_id char(10),@tr_quan int,@item_qh int,@item_pr int,@c_id char(10)
     select @tr_type=tran_type,@it_id=item_id,@tr_quan=tran_quantity,@c_id=cust_id from inserted
     select @item_qh=item_qoh from items where item_id=@it_id
     select @item_pr=item_price from items where item_id=@it_id
     if(@tr_type='s')
            begin
              update items set item_qoh=item_qoh-@tr_quan where item_id=@it_id
              update customers set sales_amnt=sales_amnt+(@item_pr*@tr_quan) where cu_id=@c_id
            end
     else
            begin
             update items set item_qoh=item_qoh+@tr_quan where item_id=@it_id
             update customers set order_amnt=order_amnt+@item_pr*@tr_quan where cu_id=@c_id
            end
   end
 drop trigger trg_transactions

insert transactions values("T000000018","a0002","ca0002","s",10,2008-04-23)
...
create proc test3 
  As
    begin
      select "customers city"=cu_city,"no. of customers"=count(cu_city),"total sold"=sum(sales_amnt) 
from customers group by cu_city having "no. of customers"=count((cu_city)<3)
   end
 exec test3 
 drop proc test3
